    ...
    fetch(`http://localhost:5000/api/product/`)
        ...
        .then((products) => {
            ...
            products.forEach((product) => {
                ...
                cardInfo.className = 'prcard-info';
                const productName = product.productName.split(' ').slice(0, 5).join(' ') + (product.productName.split(' ').length > 5 ? '...' : '');
                cardInfo.innerHTML = `
                    <h3>${productName}</h3>
